...
native_job_t::native_job_t(const std::string& event, 
    ...
    m_tag(tag)
{
    io::packed<dealer::acknowledgement> pack;
    send(pack);
}

void native_job_t::react(const events::chunk& event) {
    io::packed<dealer::chunk> pack(event.message);
    send(pack);
}

void native_job_t::react(const events::error& event) {
    io::packed<dealer::error> pack(event.code, event.message);
    send(pack);
}

void native_job_t::react(const events::choke& event) {
    io::packed<dealer::choke> pack;
    send(pack);
}

...
private:
    ...
    on_write(const Result& r) {
        m_callback(r);
        m_callback = std::function<void(boost::optional<Result>)>();
    }
...
class remote_node {
    ...
    public:
        ...
        handle(boost::optional<std::tuple<uint64_t, bool>> result) {
            ...
            if(result) {
                if(std::get<0>(*result) > m_remote.local().term()) {
...
public:
    remote_node(actor_type& local, node_id_t id):
        m_id(id),
        m_local(local),
        m_heartbeat_timer(local.service().reactor()),
    ...
    request_vote(const std::function<void(boost::optional<std::tuple<uint64_t, bool>>)>& handler) {
        ...
        m_client->call<io::raft<log_type::value_type>::vote>(
            std::make_shared<detail::result_provider<std::tuple<uint64_t, bool>>>(
                m_local.context(),
                m_id.first,
                handler
            ),
    ...
    void
    begin_leadership() {
        m_heartbeat_timer.start(0.0, float(heartbeat_timeout) / 1000.0);
    }

    void
    finish_leadership() {
        if(m_heartbeat_timer.started()) {
        ...
        if(m_local.is_leader() && m_client) {
            if(m_append_state) {
                m_client->call<io::raft<log_type::value_type>::append>(
                    ...
                    m_local.id(),
                    std::make_tuple(0, 0),
                    std::vector<log_type::value_type>(),
                ...
                auto dispatch = std::make_shared<result_provider<std::tuple<uint64_t, bool>>>(m_local.context(), m_local.id().first, handler)
                ...
                m_client->call<io::raft<log_type::value_type>::append>(
                    ...
                    std::vector<log_type::value_type>(m_local.log().iter(m_next_index), m_local.log().end()),
                    m_local.commit_index()
                );
            }
        }
        ...
        for(auto it = endpoints.begin(); it != endpoints.end(); ++it) {
            try {
                ...
                m_client = std::make_shared<client_t>(
                    io::channel<io::socket<io::tcp>>(m_local.service().reactor(), socket)
                );

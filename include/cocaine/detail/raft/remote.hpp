...
public:
    ...
    void
    replicate() {
        // TODO: Now leader sends one append request at the same time. Probably it's possible to send requests in pipeline manner. I should investigate this question.
        if(!m_append_state &&
           m_local.is_leader() &&
...
private:
    ...
    send_heartbeat() {
        if(m_client) {
            ...
            // Actually we don't need correct prev_entry to heartbeat, but the follower will not accept commit index from request with old prev_entry.
    ...
    heartbeat(ev::timer&, int) {
        if(m_local.is_leader()) {
            if(m_append_state || m_next_index > m_local.config().log().last_index()) {
                // If there is nothing to replicate or there is active append request, just send heartbeat.

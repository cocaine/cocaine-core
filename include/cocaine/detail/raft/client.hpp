...
class disposable_client_t {
    typedef typename mpl::transform<
        ...
        typename mpl::lambda<io::aux::frozen<mpl::_1>>
    >::type frozen_types;

    typedef typename boost::make_variant_over<frozen_types>::type variant_type;

    struct request_t {
        std::shared_ptr<io::dispatch_t> handler;
...
public:
         ...
         Args&&... args)
    {
        static_assert(
            ...
            "message protocol is not compatible with protocol tag"
        );
        ...
        std::function<void(const result_type&)> handler = result_handler;

        auto dispatch = make_proxy<result_type>(
            std::bind(&disposable_client_t::on_response<result_type>, this, handler, _1),
            m_context
        );

        m_operation = request_t {dispatch,
...
private:
                ...
                const boost::variant<std::error_code, CommandResult>& response)
    {
        if(boost::get<CommandResult>(&response)) {
            ...
            if(!ec) {
                ...
                m_response_handler = callback_ptr;
                COCAINE_LOG_DEBUG(m_logger, "Post result handler.");
    ...
    class operation_visitor_t :
        public boost::static_visitor<std::shared_ptr<upstream_t>>
    {
    ...
    public:
        operation_visitor_t(const std::shared_ptr<cocaine::client_t>& client,
                            const std::shared_ptr<io::dispatch_t>& handler):
            m_client(client),
            m_handler(handler)
        { }
        ...
        template<class Event>
        std::shared_ptr<upstream_t>
        operator()(const io::aux::frozen<Event>& event) const {
            return tuple::invoke(client_caller<Event> {m_client, m_handler}, event.tuple);
        }
    ...
    void
    send_request() {
        m_current_request = boost::apply_visitor(
            operation_visitor_t(m_client, m_operation->handler),
            m_operation->request
        );
    }
    ...
    ensure_connection(const node_id_t& endpoint) {
        ...
        if(m_client) {
            ...
            COCAINE_LOG_DEBUG(m_logger, "Raft client is not connected. Connecting...");
...
private:
    ...
    try_next_remote() {
        if(m_next_remote >= m_remotes.size()) {
            reset();
            COCAINE_LOG_DEBUG(m_logger, "Error!");
            auto error_handler = m_error_handler;
            error_handler();
            return;
        }
    ...
    ensure_connection(const node_id_t& endpoint) {
        if(m_current_request || m_resolver) {
            COCAINE_LOG_DEBUG(m_logger, "Some request is already processed!");
            return;
        }
        ...
        if(m_client && m_operation) {
            COCAINE_LOG_DEBUG(m_logger, "Have connection. Send request.");
            m_operation();
        } else {

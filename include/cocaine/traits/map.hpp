...
struct type_traits<std::map<K, V, C, A>> {
    template<class Stream>
    static inline
    void
    pack(msgpack::packer<Stream>& target, const std::map<K, V, C, A>& source) {
    ...
    static inline
    void
    unpack(const msgpack::object& source, std::map<K, V, C, A>& target) {

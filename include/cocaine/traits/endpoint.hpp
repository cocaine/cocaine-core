...
struct type_traits<asio::ip::basic_endpoint<InternetProtocol>> {
    ...
    typedef boost::mpl::list<std::string, unsigned short> sequence_type;

    template<class Stream>
    static inline
    void
    pack(msgpack::packer<Stream>& target, const endpoint_type& source) {
        std::string address = source.address().to_string();
        unsigned short port = source.port();

        type_traits<sequence_type>::pack(target, address, port);
    }
    ...
    unpack(const msgpack::object& source, endpoint_type& target) {
        ...
        type_traits<sequence_type>::unpack(source, address, port);

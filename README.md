...
* Планировать и асинхронно выполнять некие произвольные _задачи_:
  ...
  * [TBD] По абсолютному времени (как cron).
...
* Асинхронно и массово выполнять эти же _задачи_ (map) [TBD: и собирать их результаты в нескольких местах для агрегации (reduce)] одним запросом.

Всё общение с демоном построено на ZeroMQ, поэтому в качестве транспортов могут быть использованы следующие протоколы: IPC, TCP, UDP Multicast.
Сам демон ничего про задачи не знает - всю полезную нагрузку делают плагины, представляющие собой самые обычные .so модули, которые можно быстро и просто писать на C++ и динамически подгружать.

На данный момент, есть вот такие плагины:

* __MySQL__ - может ходить на сервер, проверять жив ли он и, если жив, получать с него количество активных процессов;
* __HTTP__ - может ходить по урлу и говорить какой там был код возврата;
* __Python__ - может выполнять какой-нибудь python-код и возвращать результат его выполнения;
* __Javascript__ - аналогично Пейтону.
...
2. Источник должен уметь:
  * Сообщить демону о том, что он умеет делать - __capabilities()__
  * Для объявленных способностей иметь соответствующие методы - __invoke()__, __reschedule()__, __process()__
 ...
 * __auto__ (параметры — _interval_, ms): дёргать __invoke()__ раз в _interval_ миллисекунд;
 * __manual__: при инициализации и после каждого запуска __invoke()__ у источника дёргается ручка __reschedule()__, которая возвращает время следующего запуска (0 — немедленно);
 * __fs__ (параметры — _path_, строка): дёргать __invoke()__ при событии файловой системы по указанному пути;
 * __zeromq__ (параметры — _endpoint_, строка): дёргать __process()__ при событии на указанном сокете, причём полученные данные будут переданы источнику для обработки;
 * __once__: см. _MapReduce_.
2. Демон создаёт новый _источник_ и запускает его треде (если треда для источника с таким же идентификатором ещё нет и в запросе не указан параметр ```{"isolated": true}```) и возвращает клиенту связанный с _источником_ и _драйвером_ ключ (и идентификатор треда в случае изолированных задач), по которому можно подписаться либо на все результаты его работы сразу, либо только на отдельные поля (либо вообще забить и не подписываться, если он нужен только ради побочных эффектов). По этому же ключу можно будет запрашивать историю.
...
1. Клиент локально или по сети говорит демону(ам), что он хочет выполнить вот такую вот задачу с драйвером __once__.
...
С недавних пор появилась кокаиновая авторизация. Если в запросе указать версию протокола >= 3 , то демон будет ожидать multi-part сообщение вида ```[json-reqest] [rsa signature]```. В самом запросе указан token, на стороне сервера в сторадже лежат публичные ключи для этих токенов. Если публичного ключа для токена не найдено, считается что юзер с этим токеном не имеет права приходить с запросами. Если ключ есть, то подпись будет провалидирована, и, если всё хорошо, запрос будет выполнен.

Distributed storage
===================

С ещё более недавних пор появилась поддержка кокаинового распределённого хранилища, а именно MongoDB. Чтобы заюзать этот прекрасный функционал, надо при запуске демону сказать --storage-driver mongo --storage-location [mongo-uri] --instance [имя инстанса]. После этого все накокаиненые демоны с одной монгой и одним именем инстанса будут ходить в шаренный сторадж за задачами, ключами и стораджами для источников.

История
=======

Для каждого //драйвера// хранится история длинной --history-depth (=10). Чтобы её получить, нужно прийти с запросом вида ```{"action": "past", "targets": {"key": {"depth": 5}}}```.
